...
const TasksPage: React.FC<TasksPageProps> = ({ onMinutesEarned, onPointsEarned }) => {
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        ...
        // Load paid tasks
        const { data: paidTasksData, error: paidTasksError } = await getActivePaidTasksForDaily();
        if (paidTasksError) {
          console.error('Error loading paid tasks:', paidTasksError);
        } else if (paidTasksData) {
          console.log('✅ Paid tasks loaded:', paidTasksData.tasks.length);
          setPaidTasks(paidTasksData.tasks);
          setCompletedPaidTasks(new Set(paidTasksData.completedTasks));
  ...
  const handlePaidTaskClick = async (taskId: string) => {
    try {
      ...
      if (result.success) {
        ...
        toast.success(
          language === 'ar'
            ? `🎉 تم إكمال المهمة المدفوعة! +${result.pointsEarned} نقطة و +${result.minutesEarned} دقيقة`
            : `🎉 Paid task completed! +${result.pointsEarned} points & +${result.minutesEarned} minutes`,
          { 
            duration: 4000,
            style: {
              background: '#00FFAA',
        ...
        toast.error(
          language === 'ar' 
            ? `فشل في إكمال المهمة: ${result.message}` 
  ...
  // Function to render a paid task card
  const renderPaidTaskCard = (task: any) => {
    const isCompleted = completedPaidTasks.has(task.id);
    
    // Determine platform info based on link
    const getPlatformInfo = (link: string) => {
      const domain = link.toLowerCase();
      
      if (domain.includes('facebook.com')) {
        return { icon: FaFacebook, color: 'border-blue-500 bg-blue-500/10', bgColor: 'bg-blue-500' };
      } else if (domain.includes('instagram.com')) {
        return { icon: FaInstagram, color: 'border-pink-500 bg-pink-500/10', bgColor: 'bg-pink-500' };
      } else if (domain.includes('twitter.com') || domain.includes('x.com')) {
        return { icon: FaXTwitter, color: 'border-sky-400 bg-sky-400/10', bgColor: 'bg-sky-400' };
      } else if (domain.includes('tiktok.com')) {
        return { icon: FaTiktok, color: 'border-pink-600 bg-pink-600/10', bgColor: 'bg-pink-600' };
      } else if (domain.includes('youtube.com')) {
        return { icon: FaYoutube, color: 'border-red-500 bg-red-500/10', bgColor: 'bg-red-500' };
      } else if (domain.includes('telegram.org') || domain.includes('t.me')) {
        return { icon: FaTelegram, color: 'border-cyan-400 bg-cyan-400/10', bgColor: 'bg-cyan-400' };
      } else {
        return { icon: Share2, color: 'border-neonGreen bg-neonGreen/10', bgColor: 'bg-neonGreen' };
      }
    };
    
    const platformInfo = getPlatformInfo(task.link);
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 mb-3">
          <PlatformIcon className={`w-6 h-6 ${platformInfo.bgColor} rounded-lg p-1 text-white`} />
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => handlePaidTaskClick(task.id)}
  ...
  return (
    <div className="min-h-screen pb-24 bg-gradient-to-b from-[#041e11] via-[#051a13] to-[#040d0c]">
          ...
          {paidTasksLoaded && paidTasks.length > 0 && (
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                {paidTasks.map(task => renderPaidTaskCard(task))}
              </div>
      ...
      <div className="mt-8 px-6">
        <div className="bg-black/40 backdrop-blur-sm border border-neonGreen/30 rounded-lg p-4">
