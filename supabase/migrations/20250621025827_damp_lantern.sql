/*
  ...
  2. Security
    ...
    - Proper validation for telegram IDs
    - Prevent self-referrals
    
  3. Testing
    - Safe test that doesn't violate foreign key constraints
    - Uses existing auth users or creates proper test scenario
*/
...
BEGIN
    ...
    ON CONFLICT (telegram_id) DO UPDATE SET
        -- تحديث supabase_auth_id دائماً للجلسة الجديدة (فقط إذا كان صالحاً)
        supabase_auth_id = CASE 
            WHEN EXCLUDED.supabase_auth_id IS NOT NULL 
                 AND EXISTS(SELECT 1 FROM auth.users WHERE id = EXCLUDED.supabase_auth_id)
...
-- اختبار آمن للنظام بدون انتهاك قيود المفاتيح الخارجية
DO $$
DECLARE
  v_test_result jsonb;
  v_existing_user_count integer;
BEGIN
  -- التحقق من وجود مستخدمين في النظام للاختبار
  SELECT COUNT(*) INTO v_existing_user_count FROM users LIMIT 2;
  
  IF v_existing_user_count >= 2 THEN
    RAISE NOTICE '✅ يوجد مستخدمون في النظام - يمكن اختبار الوظائف';
    
    -- اختبار وظيفة get_referral_stats_secure مع أول مستخدم
    SELECT get_referral_stats_secure(
    ...
    END IF;
  ELSE
    RAISE NOTICE 'ℹ️ لا يوجد مستخدمون كافيون للاختبار - سيتم الاختبار عند استخدام التطبيق';
  END IF;
  ...
  RAISE NOTICE '✅ تم إصلاح مشكلة قيود المفاتيح الخارجية';
END $$;