/*
  # إصلاح هيكل قاعدة البيانات وإعداد المستخدمين

  1. الجداول الجديدة والمحدثة
    - `users` - جدول المستخدمين مع جميع الحقول المطلوبة
    - `referrals` - جدول الإحالات
    - `daily_tasks` - المهام اليومية
    - `fixed_tasks` - المهام الثابتة
    - `mining_sessions` - جلسات التعدين
  ...
  3. الوظائف
    - تحديث وظيفة تسجيل المستخدمين
    - تحديث وظيفة معالجة الإحالات
    - معالجة المستخدمين بدون أسماء
...
-- Add basic RLS policies (without IF NOT EXISTS)
