/*
  ...
  1. التغييرات
    ...
    - تحسين وظيفة register_telegram_user للتعامل مع المستخدمين بدون أسماء
    - تحديث وظيفة get_referral_stats_secure لعرض أفضل للمستخدمين غير المسمين
    - إزالة القيود التي تمنع المستخدمين غير المسمين من المشاركة

  2. الأمان
    - الحفاظ على جميع التحققات الأمنية الموجودة
    - منع الإحالة الذاتية
    - منع الإحالات المتكررة
    - التحقق من وجود المستخدمين في النظام
*/
...
BEGIN
    ...
    ON CONFLICT (telegram_id) DO UPDATE SET
        -- تحديث supabase_auth_id دائماً للجلسة الجديدة (إذا كان صالحاً)
        supabase_auth_id = CASE 
            WHEN EXCLUDED.supabase_auth_id IS NOT NULL THEN EXCLUDED.supabase_auth_id
...
-- اختبار النظام مع مستخدمين حقيقيين (بدون إنشاء supabase_auth_id وهمي)
DO $$
DECLARE
  v_test_result jsonb;
  v_test_user_1 text := 'test_unnamed_user_' || extract(epoch from now())::text;
  v_test_user_2 text := 'test_named_user_' || extract(epoch from now())::text;
  v_user_1 users;
  v_user_2 users;
  v_referral_test jsonb;
  v_stats_test jsonb;
BEGIN
  -- إنشاء مستخدمين للاختبار (بدون supabase_auth_id لتجنب مشاكل المفاتيح الخارجية)
  SELECT register_telegram_user(v_test_user_1, NULL, NULL, 1) INTO v_user_1;
  SELECT register_telegram_user(v_test_user_2, NULL, 'مستخدم مسمى', 1) INTO v_user_2;
  
  -- التحقق من إنشاء المستخدمين بنجاح
  IF v_user_1.telegram_id IS NOT NULL AND v_user_2.telegram_id IS NOT NULL THEN
    RAISE NOTICE '✅ تم إنشاء المستخدمين للاختبار بنجاح';
    RAISE NOTICE '   - مستخدم غير مسمى: % (اسم العرض: %)', v_user_1.telegram_id, v_user_1.username;
    RAISE NOTICE '   - مستخدم مسمى: % (اسم العرض: %)', v_user_2.telegram_id, v_user_2.username;
  ELSE
    RAISE NOTICE '❌ فشل في إنشاء المستخدمين للاختبار';
    RETURN;
  END IF;
  
  -- اختبار الإحالة من مستخدم غير مسمى إلى مستخدم مسمى
  SELECT process_referral(v_user_1.telegram_id, v_user_2.telegram_id) INTO v_referral_test;
  
  IF v_referral_test->>'success' = 'true' THEN
    RAISE NOTICE '✅ اختبار الإحالة من مستخدم غير مسمى: نجح';
    RAISE NOTICE '   - تم منح % نقطة للمحيل', v_referral_test->>'points_awarded';
  ELSE
    RAISE NOTICE '❌ اختبار الإحالة من مستخدم غير مسمى: فشل - %', v_referral_test->>'message';
  END IF;
  
  -- اختبار الحصول على إحصائيات الإحالة للمستخدم غير المسمى
  SELECT get_referral_stats_secure(v_user_1.telegram_id) INTO v_stats_test;
  
  IF v_stats_test IS NOT NULL AND (v_stats_test->>'total_referrals')::integer > 0 THEN
    RAISE NOTICE '✅ اختبار إحصائيات الإحالة للمستخدم غير المسمى: نجح';
    RAISE NOTICE '   - إجمالي الإحالات: %', v_stats_test->>'total_referrals';
    RAISE NOTICE '   - الإحالات المؤكدة: %', v_stats_test->>'verified_referrals';
  ELSE
    RAISE NOTICE '❌ اختبار إحصائيات الإحالة للمستخدم غير المسمى: فشل';
  END IF;
  
  -- اختبار الإحالة العكسية (من مستخدم مسمى إلى مستخدم غير مسمى)
  -- إنشاء مستخدم ثالث للاختبار
  DECLARE
    v_test_user_3 text := 'test_unnamed_user_3_' || extract(epoch from now())::text;
    v_user_3 users;
    v_reverse_referral_test jsonb;
  BEGIN
    SELECT register_telegram_user(v_test_user_3, NULL, NULL, 1) INTO v_user_3;
    SELECT process_referral(v_user_2.telegram_id, v_user_3.telegram_id) INTO v_reverse_referral_test;
    
    IF v_reverse_referral_test->>'success' = 'true' THEN
      RAISE NOTICE '✅ اختبار الإحالة إلى مستخدم غير مسمى: نجح';
    ELSE
      RAISE NOTICE '❌ اختبار الإحالة إلى مستخدم غير مسمى: فشل - %', v_reverse_referral_test->>'message';
    END IF;
    
    -- تنظيف المستخدم الثالث
    DELETE FROM referrals WHERE referrer_id = v_user_3.telegram_id OR referred_id = v_user_3.telegram_id;
    DELETE FROM users WHERE telegram_id = v_user_3.telegram_id;
  END;
  
  -- تنظيف بيانات الاختبار
  DELETE FROM referrals WHERE referrer_id IN (v_user_1.telegram_id, v_user_2.telegram_id) OR referred_id IN (v_user_1.telegram_id, v_user_2.telegram_id);
  DELETE FROM users WHERE telegram_id IN (v_user_1.telegram_id, v_user_2.telegram_id);
  
  RAISE NOTICE '';
  RAISE NOTICE '🎉 تم تحديث نظام الإحالات بنجاح!';
  RAISE NOTICE '';
  RAISE NOTICE '✅ المستخدمون غير المسمون يمكنهم الآن:';
  RAISE NOTICE '   - إحالة مستخدمين آخرين (مسمين أو غير مسمين)';
  RAISE NOTICE '   - أن يتم إحالتهم من قبل مستخدمين آخرين';
  RAISE NOTICE '   - الحصول على مكافآت الإحالة (30 نقطة فوراً + 60 دقيقة عند المطالبة)';
  ...
  RAISE NOTICE '   - جميع التحققات الأمنية محفوظة (منع الإحالة الذاتية والمتكررة)';
  RAISE NOTICE '   - دعم كامل للمستخدمين بدون أسماء مستخدم';
  RAISE NOTICE '   - تحسين عرض البيانات في واجهة المستخدم';
  RAISE NOTICE '';
  RAISE NOTICE '🔒 الأمان:';
  RAISE NOTICE '   - لا يمكن للمستخدم إحالة نفسه';
  RAISE NOTICE '   - لا يمكن إحالة مستخدم تم إحالته مسبقاً';
  RAISE NOTICE '   - التحقق من وجود المستخدمين في النظام قبل الإحالة';
  RAISE NOTICE '   - جميع المكافآت محمية من التلاعب';
  
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE '❌ حدث خطأ أثناء الاختبار: %', SQLERRM;
    -- تنظيف في حالة الخطأ
    DELETE FROM referrals WHERE referrer_id LIKE 'test_%' OR referred_id LIKE 'test_%';
