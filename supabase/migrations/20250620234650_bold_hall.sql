...
-- اختبار النظام مع مستخدم غير مسمى
DO $$
DECLARE
  v_test_result jsonb;
  v_test_user_1 text := 'test_user_unnamed_1';
  v_test_user_2 text := 'test_user_unnamed_2';
  v_auth_id_1 uuid := gen_random_uuid();
  v_auth_id_2 uuid := gen_random_uuid();
  v_user_1 users;
  v_user_2 users;
BEGIN
  -- إنشاء مستخدمين للاختبار (أحدهما بدون username)
  SELECT register_telegram_user(v_test_user_1, v_auth_id_1, NULL, 1) INTO v_user_1;
  SELECT register_telegram_user(v_test_user_2, v_auth_id_2, 'مستخدم مسمى', 1) INTO v_user_2;
