/*
  ...
  1. تحديث الوظائف
    - تحديث process_referral لقبول المستخدمين غير المسمين
    - تحديث register_telegram_user لتعامل أفضل مع المستخدمين غير المسمين
...
-- اختبار النظام بطريقة آمنة بدون استخدام متغيرات من نوع users
DO $$
DECLARE
  v_test_result jsonb;
  v_test_user_1 text := 'test_user_unnamed_1';
  v_test_user_2 text := 'test_user_unnamed_2';
  v_user_1_result jsonb;
  v_user_2_result jsonb;
  v_user_exists boolean;
BEGIN
  -- إنشاء مستخدمين للاختبار (بدون supabase_auth_id لتجنب انتهاك القيود)
  SELECT register_telegram_user(v_test_user_1, NULL, NULL, 1) INTO v_user_1_result;
  SELECT register_telegram_user(v_test_user_2, NULL, 'مستخدم مسمى', 1) INTO v_user_2_result;
  
  -- التحقق من نجاح إنشاء المستخدمين
  SELECT EXISTS(SELECT 1 FROM users WHERE telegram_id = v_test_user_1) INTO v_user_exists;
  
  IF v_user_exists THEN
    RAISE NOTICE '✅ تم إنشاء المستخدم الأول بنجاح';
    
    SELECT EXISTS(SELECT 1 FROM users WHERE telegram_id = v_test_user_2) INTO v_user_exists;
    
    IF v_user_exists THEN
      RAISE NOTICE '✅ تم إنشاء المستخدم الثاني بنجاح';
