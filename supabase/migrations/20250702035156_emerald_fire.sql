/*
  # إصلاح نظام التعدين والجلسات والمهام اليومية

  1. تحديثات جدول المستخدمين
    - إضافة تتبع جلسات التعدين
    - تحسين تتبع الجلسات اليومية

  2. جدول جلسات التعدين الجديد
    - تتبع دقيق لجلسات التعدين
    - حالات مختلفة للجلسات
    - منع التلاعب

  3. تحديث المهام اليومية
    - إضافة روابط جديدة للمنصات
    - تحديث المحتوى والأوصاف

  4. وظائف محسنة
    - نظام تعدين محسن
    - تتبع دقيق للجلسات اليومية
    - إحصائيات إحالة محسنة
*/
...
-- حذف المهام المكتملة أولاً لتجنب انتهاك قيود المفاتيح الخارجية
DELETE FROM user_daily_tasks WHERE daily_task_id IN (
  SELECT id FROM daily_tasks
);

-- الآن يمكن حذف المهام اليومية بأمان
