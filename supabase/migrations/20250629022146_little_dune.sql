/*
  ...
  1. تحديث الوظائف
    - `process_referral` - قبول المستخدمين مع أو بدون username
    - `register_telegram_user` - تحسين التعامل مع المستخدمين غير المسمين
    - `get_referral_stats_secure` - عرض أفضل للمستخدمين غير المسمين
...
BEGIN
    ...
    -- إرجاع معلومات المستخدم كـ JSON
    SELECT jsonb_build_object(
        ...
        'registration_bonus_applied', registration_bonus_applied
    ) INTO v_result
    FROM users
...
BEGIN
  ...
  IF v_user_1_result IS NOT NULL AND v_user_2_result IS NOT NULL THEN
