/*
  # Update referral system to support unnamed users

  1. Updates
    - Update process_referral function to accept unnamed users
    - Update register_telegram_user function for better unnamed user handling
    - Update get_referral_stats_secure function for better display of unnamed users
    - Fix foreign key constraint issues in testing
  ...
  3. Testing
    - Safe test that doesn't violate foreign key constraints
...
-- اختبار النظام مع مستخدم غير مسمى (بدون انتهاك قيود المفاتيح الخارجية)
DO $$
DECLARE
  v_test_result jsonb;
  v_test_user_1 text := 'test_user_unnamed_1';
  v_test_user_2 text := 'test_user_unnamed_2';
  v_user_created boolean;
BEGIN
  -- إنشاء مستخدمين للاختبار (بدون supabase_auth_id لتجنب انتهاك القيود)
  -- إنشاء المستخدم الأول (غير مسمى)
  PERFORM register_telegram_user(v_test_user_1, NULL, NULL, 1);
  
  -- التحقق من إنشاء المستخدم الأول
  SELECT EXISTS(SELECT 1 FROM users WHERE telegram_id = v_test_user_1) INTO v_user_created;
  IF NOT v_user_created THEN
    RAISE NOTICE '❌ فشل في إنشاء المستخدم الأول';
    RETURN;
  END IF;
  
  -- إنشاء المستخدم الثاني (مسمى)
  PERFORM register_telegram_user(v_test_user_2, NULL, 'مستخدم مسمى', 1);
  
  -- التحقق من إنشاء المستخدم الثاني
  SELECT EXISTS(SELECT 1 FROM users WHERE telegram_id = v_test_user_2) INTO v_user_created;
  IF NOT v_user_created THEN
    RAISE NOTICE '❌ فشل في إنشاء المستخدم الثاني';
    -- تنظيف المستخدم الأول
    DELETE FROM users WHERE telegram_id = v_test_user_1;
    RETURN;
  END IF;
  
  -- اختبار الإحالة من مستخدم غير مسمى إلى مستخدم مسمى
  SELECT process_referral(v_test_user_1, v_test_user_2) INTO v_test_result;
  
  IF v_test_result->>'success' = 'true' THEN
    RAISE NOTICE '✅ اختبار الإحالة من مستخدم غير مسمى: نجح';
  ELSE
    RAISE NOTICE '❌ اختبار الإحالة من مستخدم غير مسمى: فشل - %', v_test_result->>'message';
  END IF;
  
  -- اختبار الحصول على إحصائيات الإحالة للمستخدم غير المسمى
  SELECT get_referral_stats_secure(v_test_user_1) INTO v_test_result;
  
  IF v_test_result IS NOT NULL AND (v_test_result->>'total_referrals')::integer > 0 THEN
  ...
  DELETE FROM referrals WHERE referrer_id IN (v_test_user_1, v_test_user_2) OR referred_id IN (v_test_user_1, v_test_user_2);
  DELETE FROM users WHERE telegram_id IN (v_test_user_1, v_test_user_2);
