/*
  # LYRA Task Consumption System

  1. New Tables
    - `task_clicks` - Tracks individual clicks on tasks
      - `id` (uuid, primary key)
      - `task_id` (uuid, foreign key to paid_tasks)
      - `user_telegram_id` (text, foreign key to users)
      - `clicked_at` (timestamp)
      - `lyra_consumed` (numeric)
    
    - `task_click_batches` - Tracks batches of clicks for processing
      - `id` (uuid, primary key)
      - `task_id` (uuid, foreign key to paid_tasks)
      - `clicks_count` (integer)
      - `total_lyra_consumed` (numeric)
      - `processed_at` (timestamp)
  
  2. Functions
    - `record_task_click` - Records a click on a task and consumes LYRA
    - `process_click_batch` - Processes a batch of clicks
    - `update_task_completion_status` - Updates task status when completed
    
  3. Triggers
    - `update_task_status_on_click` - Updates task status when clicks reach target
*/
...
-- Allow users to read their own task clicks
...
-- Allow users to read clicks on their tasks
...
-- Allow task owners to read batches for their tasks
