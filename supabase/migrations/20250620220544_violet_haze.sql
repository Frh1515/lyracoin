/*
  ...
  1. التغييرات
    - تحديث المهام الثابتة الموجودة لتحتوي على النصوص بكلا اللغتين
    - إضافة المهام اليومية مع دعم اللغتين
    - استخدام تنسيق "النص العربي | English Text"

  2. الأمان
    - تجنب التضارب في الإدراج
    - التحقق من وجود المهام قبل الإضافة
...
-- حذف المهام اليومية الموجودة لتجنب التضارب
DELETE FROM public.daily_tasks;

-- إنشاء المهام اليومية مع دعم اللغتين
...
BEGIN
    ...
    IF fixed_count != 6 THEN
        RAISE WARNING '⚠️ عدد المهام الثابتة يجب أن يكون 6، العدد الحالي: %', fixed_count;
    END IF;
    
    IF daily_count != 10 THEN
        RAISE WARNING '⚠️ عدد المهام اليومية يجب أن يكون 10، العدد الحالي: %', daily_count;
    END IF;
END $$;