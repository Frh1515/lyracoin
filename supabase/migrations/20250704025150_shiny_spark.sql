/*
  ...
  3. Functions
    - Update register_telegram_user function
    - Update process_referral function
    - Handle unnamed users properly
...
CREATE TABLE IF NOT EXISTS referral_claims (
  ...
  minutes_claimed integer DEFAULT 60,
...
DROP POLICY IF EXISTS "Anyone can read daily tasks" ON daily_tasks;
CREATE POLICY "Anyone can read daily tasks" ON daily_tasks
  FOR SELECT TO public
  USING (is_active = true);

DROP POLICY IF EXISTS "Anyone can read fixed tasks" ON fixed_tasks;
CREATE POLICY "Anyone can read fixed tasks" ON fixed_tasks
  FOR SELECT TO public
  USING (is_active = true);

DROP POLICY IF EXISTS "Users can read their own mining sessions" ON mining_sessions;
CREATE POLICY "Users can read their own mining sessions" ON mining_sessions
  FOR SELECT TO authenticated
  USING (user_telegram_id IN (
    SELECT telegram_id FROM users WHERE supabase_auth_id = auth.uid()
  ));

DROP POLICY IF EXISTS "Users can insert their own mining sessions" ON mining_sessions;
...
DROP POLICY IF EXISTS "Users can update their own mining sessions" ON mining_sessions;
...
DROP POLICY IF EXISTS "Users can read their own daily task completions" ON user_daily_tasks;
CREATE POLICY "Users can read their own daily task completions" ON user_daily_tasks
  FOR SELECT TO authenticated
  USING (user_telegram_id IN (
    SELECT telegram_id FROM users WHERE supabase_auth_id = auth.uid()
  ));

DROP POLICY IF EXISTS "Users can insert their own daily task completions" ON user_daily_tasks;
CREATE POLICY "Users can insert their own daily task completions" ON user_daily_tasks
  FOR INSERT TO authenticated
  WITH CHECK (user_telegram_id IN (
    SELECT telegram_id FROM users WHERE supabase_auth_id = auth.uid()
  ));

DROP POLICY IF EXISTS "Users can read their own fixed task completions" ON user_fixed_tasks;
CREATE POLICY "Users can read their own fixed task completions" ON user_fixed_tasks
  FOR SELECT TO authenticated
  USING (user_telegram_id IN (
    SELECT telegram_id FROM users WHERE supabase_auth_id = auth.uid()
  ));

DROP POLICY IF EXISTS "Users can insert their own fixed task completions" ON user_fixed_tasks;
CREATE POLICY "Users can insert their own fixed task completions" ON user_fixed_tasks
  FOR INSERT TO authenticated
  WITH CHECK (user_telegram_id IN (
    SELECT telegram_id FROM users WHERE supabase_auth_id = auth.uid()
  ));
...
DROP POLICY IF EXISTS "Users can read their own claims" ON referral_claims;
CREATE POLICY "Users can read their own claims" ON referral_claims
  FOR SELECT TO authenticated
  USING (claimer_telegram_id IN (
    SELECT telegram_id FROM users WHERE supabase_auth_id = auth.uid()
  ));

DROP POLICY IF EXISTS "Users can insert their own claims" ON referral_claims;
CREATE POLICY "Users can insert their own claims" ON referral_claims
  FOR INSERT TO authenticated
  WITH CHECK (claimer_telegram_id IN (
    SELECT telegram_id FROM users WHERE supabase_auth_id = auth.uid()
  ));
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'user_daily_tasks_user_telegram_id_fkey'
  ) THEN
    ALTER TABLE user_daily_tasks ADD CONSTRAINT user_daily_tasks_user_telegram_id_fkey 
    FOREIGN KEY (user_telegram_id) REFERENCES users(telegram_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'user_daily_tasks_daily_task_id_fkey'
  ) THEN
    ALTER TABLE user_daily_tasks ADD CONSTRAINT user_daily_tasks_daily_task_id_fkey 
    FOREIGN KEY (daily_task_id) REFERENCES daily_tasks(id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'user_fixed_tasks_user_telegram_id_fkey'
  ) THEN
    ALTER TABLE user_fixed_tasks ADD CONSTRAINT user_fixed_tasks_user_telegram_id_fkey 
    FOREIGN KEY (user_telegram_id) REFERENCES users(telegram_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'user_fixed_tasks_fixed_task_id_fkey'
  ) THEN
    ALTER TABLE user_fixed_tasks ADD CONSTRAINT user_fixed_tasks_fixed_task_id_fkey 
    FOREIGN KEY (fixed_task_id) REFERENCES fixed_tasks(id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'mining_sessions_user_telegram_id_fkey'
  ) THEN
    ALTER TABLE mining_sessions ADD CONSTRAINT mining_sessions_user_telegram_id_fkey 
    FOREIGN KEY (user_telegram_id) REFERENCES users(telegram_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'game_sessions_user_telegram_id_fkey'
  ) THEN
    ALTER TABLE game_sessions ADD CONSTRAINT game_sessions_user_telegram_id_fkey 
    FOREIGN KEY (user_telegram_id) REFERENCES users(telegram_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'referral_claims_referral_id_fkey'
  ) THEN
    ALTER TABLE referral_claims ADD CONSTRAINT referral_claims_referral_id_fkey 
    FOREIGN KEY (referral_id) REFERENCES referrals(id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'referral_claims_claimer_telegram_id_fkey'
  ) THEN
    ALTER TABLE referral_claims ADD CONSTRAINT referral_claims_claimer_telegram_id_fkey 
